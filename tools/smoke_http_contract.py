...
def _run_sequence(config: SmokeConfig) -> None:
    ...
    print(f"[smoke] schemas OK (using schema={schema})")

    tables_url = f"{config.base_url}/v1/dialects/{config.dialect}/schemas/{schema}/tables"
    ...
    table = config.table or (tables[0] if tables else "")
    _require(bool(table), "no table available for describe endpoint")
    print(f"[smoke] tables OK (using table={table})")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--schema",
        default=os.getenv("SCRATCHBIRD_AI_SMOKE_SCHEMA", "public"),
    )
    parser.add_argument(
        "--table",
        default=os.getenv("SCRATCHBIRD_AI_SMOKE_TABLE", "customers"),
    )
    parser.add_argument(
        "--database",
