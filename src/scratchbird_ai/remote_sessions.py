    ...
    def open_session(
        ...
        *,
        ...
        return {
            ...
            "warnings": [
                "mcp_remote_v0 remains preview-grade until transport certification and streaming support land"
            ],
        }
    ...
    def _authenticate(self, auth_envelope: Any, security_context_hint: Any = None) -> dict[str, Any]:
        if self.auth_token is None:
            raise ToolContractError(
                ...
                message="remote MCP auth is not configured",
        ...
        auth_type = str(auth_envelope.get("auth_type", "")).strip() or "bearer"
        if auth_type != "bearer":
            raise ToolContractError(
        ...
        token = str(auth_envelope.get("token", "")).strip()
